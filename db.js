...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS customer_phone TEXT; ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_method TEXT; ALTER TABLE orders ADD COLUMN IF NOT EXISTS product_id INTEGER; ALTER TABLE orders ADD COLUMN IF NOT EXISTS qty INTEGER DEFAULT 1; ALTER TABLE orders ADD COLUMN IF NOT EXISTS unit_price NUMERIC(14,2) DEFAULT 0; ALTER TABLE orders ADD COLUMN IF NOT EXISTS cost_price NUMERIC(14,2) DEFAULT 0;
CREATE TABLE IF NOT EXISTS products(id SERIAL PRIMARY KEY,title TEXT NOT NULL,sku TEXT,quantity INTEGER DEFAULT 0,price NUMERIC(14,2) DEFAULT 0,raw JSONB,updated_at TIMESTAMPTZ DEFAULT NOW()); ALTER TABLE products ADD COLUMN IF NOT EXISTS size TEXT; ALTER TABLE products ADD COLUMN IF NOT EXISTS cost_price NUMERIC(14,2) DEFAULT 0; ALTER TABLE products ADD COLUMN IF NOT EXISTS low_stock_level INTEGER DEFAULT 3;
...
CREATE TABLE IF NOT EXISTS suppliers(id SERIAL PRIMARY KEY,name TEXT NOT NULL,phone TEXT,email TEXT,note TEXT,created_at TIMESTAMPTZ DEFAULT NOW());
CREATE TABLE IF NOT EXISTS stock_movements(id SERIAL PRIMARY KEY,product_id INTEGER,product_title TEXT,sku TEXT,type TEXT NOT NULL,qty INTEGER NOT NULL,note TEXT,created_at TIMESTAMPTZ DEFAULT NOW());
CREATE TABLE IF NOT EXISTS returns(id SERIAL PRIMARY KEY,order_id INTEGER,product_id INTEGER,qty INTEGER DEFAULT 1,type TEXT DEFAULT 'return',reason TEXT,created_at TIMESTAMPTZ DEFAULT NOW());
...
`);
if(process.env.ADMIN_EMAIL&&process.env.ADMIN_PASSWORD){const r=await pool.query("SELECT id FROM users WHERE email=$1",[process.env.ADMIN_EMAIL]);if(!r.rowCount){const h=await bcrypt.hash(process.env.ADMIN_PASSWORD,12);await pool.query("INSERT INTO users(email,password_hash) VALUES($1,$2)",[process.env.ADMIN_EMAIL,h]);}}
}
